...
col = 1
for i in range(1, 11):
    wheel_data = pd.read_csv(wheelGlove[i], sep=';')  ################# CHANGE wheelController <-> wheelGlove
    ...
    fig.add_scatter3d(x=x_data_pos_virtual_l, y=y_data_pos_virtual_l, z=z_data_pos_virtual_l,
                      mode='markers',
                      marker=dict(color=time_l, colorscale='Magenta', colorbar=dict(thickness=20, x=1.2)))
    ...
    figMain.add_trace(go.Scatter3d(x=x_data_pos_virtual_r, y=y_data_pos_virtual_r, z=z_data_pos_virtual_r,
                                   mode='markers',
                                   marker=dict(color=time_r, colorscale='Peach', colorbar=dict(thickness=20, x=1.3))),
                      row=row, col=col)
