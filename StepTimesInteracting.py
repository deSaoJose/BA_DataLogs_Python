...
def makeStuff(lines, i):
    ...
    stuff["time"].append(convertStringToFloatSeconds(lines[2].split(";")[2]))

    stuff["input"].append(i)
    stuff["task"].append("Ziehe die Feststellbremse an.")
    stuff["time"].append(convertStringToFloatSeconds(lines[4].split(";")[2]))

    stuff["input"].append(i)
    stuff["task"].append("Ziehe die Druckluftbremse an.")
    stuff["time"].append(convertStringToFloatSeconds(lines[6].split(";")[2]))

    stuff["input"].append(i)
    stuff["task"].append("Prüfe, ob das Fahrzeug nun angebremst ist.")
    stuff["time"].append(convertStringToFloatSeconds(lines[8].split(";")[2]))

    stuff["input"].append(i)
    stuff["task"].append("Signalisiere der Lok, dass sie anfahren darf.")
    stuff["time"].append(convertStringToFloatSeconds(lines[9].split(";")[2]))

    stuff["input"].append(i)
    stuff["task"].append("Kupple den vordersten Wagen an die Lok.")
    stuff["time"].append(convertStringToFloatSeconds(lines[13].split(";")[2]))
